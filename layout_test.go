...
import (
	...
	"github.com/zap-proto/zip/internal/layout"
)
...
func TestTheLayoutIsCheckedIn(t *testing.T) {
	got, err := zip.Layouts(reflect.TypeOf(layout.Trunk{}), reflect.TypeOf(layout.Ided{}))
	if err != nil {
		t.Fatal(err)
	}
	if len(got) != 1 {
	...
	path := filepath.Join("internal", "layout", "zap_gen.go")
	...
	if string(got[0].Source) != string(want) {
		...
		t.Errorf("%s is stale; regenerate with WRITE=1 go test -run TestTheLayoutIsCheckedIn ./...", path)
	}
}
...
func TestTheEmittedSourceCompiles(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("go build ./internal/layout: %v\n%s", err, out)
	}
}
