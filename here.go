...
// so plainly.
//
// A PARKED OP IS NOT AN OUT. When the authorizer approves rather than allows, the
// handler does not run and there is no reply to hand back — so the [Approval]
// itself comes back on the error lane, and [HeldOf] reads it. That is the same
// answer the 202 carries over REST and the call plane: one value, four
// projections, and no way to read a held op as a finished one.
func Here[In, Out any](ctx context.Context, a *App, op string, in *In) (*Out, error) {
	...
	// Held, not mistyped. Surfacing the Approval before the assertion is what keeps
	// a policy outcome from being reported as the caller's type mistake.
