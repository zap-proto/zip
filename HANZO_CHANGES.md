...
| Adapters | `AdaptNetHTTP / AdaptNetHTTPMiddleware`; front a foreign subtree via `app.Raw(zip.MethodAll, prefix+"/*", zip.AdaptNetHTTP(h))`. A bare func adapts as `AdaptNetHTTP(http.HandlerFunc(fn))` — `http.HandlerFunc` IS an `http.Handler`, so there is no separate func adapter. |
