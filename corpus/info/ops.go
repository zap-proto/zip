...
func (i *Info) Ops() *zip.App {
	...
	app.Get("/node/version", i.nodeVersion)
	app.Get("/node/id", i.identity)
	app.Get("/node/ip", i.address)
	app.Get("/network/id", i.networkID)
	app.Get("/network/name", i.networkName)
	app.Get("/chain/id", i.chainID)
	app.Get("/chain/bootstrapped", i.bootstrapped)
	app.Get("/chains", i.chains)
	app.Get("/peers", i.peers)
	app.Get("/lps", i.lps)
	app.Get("/vms", i.vms)
	app.Get("/upgrades", i.upgrades)
	app.Get("/uptime", i.uptime)
	app.Get("/fees", i.fees)
	return app
}
