...
`zip.Codecs(roots...)` renders the `MarshalZAP`/`UnmarshalZAP` pair for each
...
its wire without taking on this one's dependencies. The restatement is a
compile-time assertion per type, which is what makes a deleted codec a build
failure rather than a silent return to reflection.

**The wire does not move**, and that is held, not asserted:
`internal/codec.TestTheCodecKeepsTheWire` requires the generated bytes to equal
...
the socket peer by default: `X-Forwarded-For` is attacker-controlled
input, and an IP that silently reports the load balancer is worse than
none, because someone will rate-limit or audit on it. A deployment behind
a proxy opts in explicitly and the opt-in is allowlist-gated. Empty when
no connection is behind the context.
